---
title: Directorio de personas creadoras con discapacidad
description: Un directorio de personas con discapacidad que son artistas, escritoras, músicas, cineastas, performers y creadoras en todos los medios.
published: true
date: 2026-06-05T20:20:44.291Z
tags: 
editor: markdown
dateCreated: 2025-11-20T19:07:47.911Z
---

# Directorio de personas creadoras con discapacidad

Un directorio de personas con discapacidad que son artistas, escritoras, músicas, cineastas, performers y creadoras en todos los medios. Esta página te ayuda a descubrir y apoyar a personas creadoras con discapacidad.

---

## Sobre este directorio

Este directorio reúne en un solo lugar a personas creadoras con discapacidad para facilitar su descubrimiento. Todas las personas creadoras que figuran aquí son abiertamente personas con discapacidad y crean obra que puede estar o no centrada en la discapacidad.

**Este directorio está en constante crecimiento.** Sugiere agregados a través de [Cómo Contribuir](/es/glossary/how-to-contribute).

**Etiquetas usadas abajo**
- *(fuera de EE. UU.)* — la persona creadora está radicada fuera de Estados Unidos

---

## Contenido

- [Escritores y autores](#escritores-y-autores)
- [Personas artistas visuales](#personas-artistas-visuales)
- [Músicos y compositores](#músicos-y-compositores)
- [Cineastas y directores](#cineastas-y-directores)
- [Actores y performers](#actores-y-performers)
- [Comediantes](#comediantes)
- [Bailarines y coreógrafos](#bailarines-y-coreógrafos)
- [Pódcasters y creadores de audio](#pódcasters-y-creadores-de-audio)
- [YouTubers y creadores de video](#youtubers-y-creadores-de-video)
- [Periodistas y ensayistas](#periodistas-y-ensayistas)
- [Académicos y estudiosos](#académicos-y-estudiosos)
- [Activistas que crean](#activistas-que-crean)
- [Cómo apoyar a las personas creadoras con discapacidad](#cómo-apoyar-a-las-personas-creadoras-con-discapacidad)

---

## Escritores y autores {#escritores-y-autores}

### Escritores de ficción

| Nombre | Discapacidad | Género/Enfoque | Obra notable |
|------|------------|-------------|--------------|
| Susan Nussbaum | Usuaria de silla de ruedas | Ficción literaria | Good Kings Bad Kings |
| Rivers Solomon | Autista, enfermedad crónica | Ficción especulativa | An Unkindness of Ghosts |
| Nicola Griffith | EM | Literaria/suspenso | So Lucky |
| Talia Hibbert | Fibromialgia, enfermedad crónica | Romance | Get a Life, Chloe Brown |
| Helen Hoang | Autista | Romance | The Kiss Quotient |
| Leigh Bardugo | Osteonecrosis | Fantasía | Six of Crows |
| Corinne Duyvis | Autista | Ciencia ficción para jóvenes adultos | On the Edge of Gone |
| Elle McNicoll | Autista | Lectores intermedios | A Kind of Spark *(fuera de EE. UU.)* |
| Sara Nović | Sorda | Ficción literaria | True Biz |
| Ann Clare LeZotte | Sorda | Lectores intermedios | Show Me a Sign |
| Harriet McBryde Johnson | Discapacidad neuromuscular | Jóvenes adultos/ensayos | Accidents of Nature |
| Ivelisse Housman | Autista | Fantasía para jóvenes adultos | Unseelie |

### Escritores de no ficción

| Nombre | Discapacidad | Enfoque | Obra notable |
|------|------------|-------|--------------|
| Alice Wong | AME, usuaria de silla de ruedas | Cultura de la discapacidad | Disability Visibility |
| Leah Lakshmi Piepzna-Samarasinha | Enfermedad crónica, neurodivergente | Justicia para las personas con discapacidad | Care Work |
| Emily Ladau | Usuaria de silla de ruedas | Educación/defensa de derechos | Demystifying Disability |
| Judith Heumann | Polio, usuaria de silla de ruedas | Memoria/activismo | Being Heumann |
| Haben Girma | Sordociega | Memoria | Haben |
| Eli Clare | Discapacidad física, genderqueer | Teoría/memoria | Brilliant Imperfection |
| Imani Barbarin | Parálisis cerebral | Ensayos/defensa de derechos | Crutches and Spice |
| Esmé Weijun Wang | Trastorno esquizoafectivo | Ensayos | The Collected Schizophrenias |
| Rebekah Taussig | Usuaria de silla de ruedas | Memoria | Sitting Pretty |
| Simi Linton | Usuaria de silla de ruedas | Memoria/teoría | My Body Politic |

### Poetas

| Nombre | Discapacidad | Estilo | Obra notable |
|------|------------|-------|--------------|
| Jim Ferris | Discapacidad física | Poesía crip | The Hospital Poems |
| Kenny Fries | Diferencia en una extremidad | Poesía/memoria | Body, Remember |
| Petra Kuppers | Enfermedad crónica | Poesía de performance | Cripple Poetics |
| Neil Marcus | Discapacidad física | Performance | Storm Reading |
| Vassar Miller | Parálisis cerebral | Verso tradicional | Collected Poems |

---

## Personas artistas visuales {#personas-artistas-visuales}

| Nombre | Discapacidad | Medio | Notable por |
|------|------------|--------|-------------|
| Frida Kahlo (1907-1954) | Daño en la columna, dolor crónico | Pintura | Autorretratos, ícono de la discapacidad |
| Riva Lehrer | Espina bífida | Pintura | Retratos de Circle Stories |
| Sunaura Taylor | Artrogriposis | Pintura, escritura | Derechos de los animales/de la discapacidad |
| Christine Sun Kim | Sorda | Arte sonoro, dibujo | Explorar el sonido y el silencio |
| Joseph Grigely | Sordo | Arte conceptual | Conversation pieces |
| Katherine Sherwood | Sobreviviente de derrame cerebral | Pintura | Obra posterior al derrame |
| Yinka Shonibare | Discapacidad física | Instalación | Obra sobre identidad cultural *(fuera de EE. UU.)* |
| Sandie Yi | Discapacidad física | Escultura | Arte de instalación |
| Chun-Shan (Sandie) Yi | Usuaria de silla de ruedas | Escultura | Artes de la discapacidad |
| Tony Heaton | Usuario de silla de ruedas | Escultura | Artes de la discapacidad *(fuera de EE. UU.)* |

---

## Músicos y compositores {#músicos-y-compositores}

| Nombre | Discapacidad | Género | Notable por |
|------|------------|-------|-------------|
| Stevie Wonder | Ciego | Soul/R&B | Carrera legendaria |
| Ray Charles (1930-2004) | Ciego | R&B/soul | Pionero |
| Evelyn Glennie | Sorda | Percusión clásica | Solista internacional *(fuera de EE. UU.)* |
| Sean Forbes | Sordo | Hip-hop | Fundador de D-PAN |
| Mandy Harvey | Sordera tardía | Cantautora | Finalista de AGT |
| Itzhak Perlman | Polio | Violín clásico | Violinista de renombre mundial |
| Teddy Pendergrass (1950-2010) | Paralizado | Soul | Continuó actuando tras su lesión |
| Django Reinhardt (1910-1953) | Parálisis parcial de la mano | Guitarra de jazz | Desarrolló una técnica única |
| Viktoria Modesta | Amputada | Pop/performance | Artista biónica *(fuera de EE. UU.)* |
| Gaelynn Lea | Osteogénesis imperfecta | Folk indie | Ganadora del NPR Tiny Desk |

---

## Cineastas y directores {#cineastas-y-directores}

| Nombre | Discapacidad | Enfoque | Obra notable |
|------|------------|-------|--------------|
| Jim LeBrecht | Usuario de silla de ruedas | Documental | Crip Camp (codirector) |
| Jennifer Brea | EM/SFC | Documental | Unrest |
| Jenni Gold | Usuaria de silla de ruedas | Documental | CinemAbility |
| Rodney Evans | Pérdida de visión | Documental | Vision Portraits |
| Billy Golfus | Lesión cerebral | Documental | When Billy Broke His Head |
| Shoshannah Stern | Sorda | Creadora de TV | This Close |
| Josh Feldman | Sordo | Creador de TV | This Close |
| Ryan O'Connell | Parálisis cerebral, gay | Creador/actor de TV | Special |

---

## Actores y performers {#actores-y-performers}

### Actores de cine y TV

| Nombre | Discapacidad | Papeles notables |
|------|------------|---------------|
| Marlee Matlin | Sorda | Children of a Lesser God, Switched at Birth |
| Troy Kotsur | Sordo | CODA (ganador del Óscar) |
| Millicent Simmonds | Sorda | A Quiet Place, Wonderstruck |
| Lauren Ridloff | Sorda | Eternals, The Walking Dead |
| RJ Mitte | Parálisis cerebral | Breaking Bad |
| Micah Fowler | Parálisis cerebral | Speechless |
| Zack Gottsagen | Síndrome de Down | The Peanut Butter Falcon |
| Jamie Brewer | Síndrome de Down | American Horror Story |
| Danny Woodburn | Persona de talla baja | Seinfeld, varios |
| Peter Dinklage | Acondroplasia | Game of Thrones |
| Warwick Davis | Displasia espondiloepifisaria | Harry Potter, Willow *(fuera de EE. UU.)* |
| Ali Stroker | Usuaria de silla de ruedas | Oklahoma! (ganadora del Tony) |
| Madison Ferris | Distrofia muscular | The Glass Menagerie (Broadway) |
| Kiera Allen | Usuaria de silla de ruedas | Run |
| Alaqua Cox | Sorda, amputada | Echo (Marvel) |
| Mat Fraser | Focomelia | American Horror Story *(fuera de EE. UU.)* |

### Performers de teatro

| Nombre | Discapacidad | Notable por |
|------|------------|-------------|
| Ali Stroker | Usuaria de silla de ruedas | Primera usuaria de silla de ruedas en ganar un Tony |
| John McGinty | Sordo | Spring Awakening de Broadway |
| Sandra Mae Frank | Sorda | Spring Awakening, Wonderstruck |
| Russell Harvard | Sordo | Tribes, varios |
| Daniel Durant | Sordo | CODA, Spring Awakening |

---

## Comediantes {#comediantes}

| Nombre | Discapacidad | Plataforma | Notable por |
|------|------------|----------|-------------|
| Maysoon Zayid | Parálisis cerebral | Stand-up | Charla TED viral |
| Josh Blue | Parálisis cerebral | Stand-up | Ganador de Last Comic Standing |
| Zach Anner | Parálisis cerebral | Stand-up, YouTube | Ganador del programa de Oprah |
| Drew Lynch | Tartamudez | Stand-up | Finalista de AGT |
| Lee Ridley (Lost Voice Guy) | Usuario de CAA | Stand-up | Ganador de BGT *(fuera de EE. UU.)* |
| Adam Hills | Amputado | Conductor de TV | The Last Leg *(fuera de EE. UU.)* |
| Francesca Martinez | Parálisis cerebral | Stand-up | Comediante británica *(fuera de EE. UU.)* |
| Alex Brooker | Diferencia en una extremidad | Conductor de TV | The Last Leg *(fuera de EE. UU.)* |
| Nina G | Tartamudez, dislexia | Stand-up | Comediante con tartamudez |

---

## Bailarines y coreógrafos {#bailarines-y-coreógrafos}

| Nombre | Discapacidad | Compañía/Estilo | Notable por |
|------|------------|---------------|-------------|
| Alice Sheppard | Usuaria de silla de ruedas | Kinetic Light | Coreógrafa, performer |
| Judith Smith | Discapacidad física | AXIS Dance | Cofundadora |
| Laurel Lawson | Usuaria de silla de ruedas | Kinetic Light | Performer |
| Marc Brew | Discapacidad física | Coreógrafo | Trabajo internacional *(fuera de EE. UU.)* |
| Alito Alessi | Discapacidad física | DanceAbility | Fundador de la metodología |
| Chelsie Hill | Paralizada | Rollettes | Equipo de danza en silla de ruedas |
| Bill Shannon | Discapacidad física | Bailarín con muletas | Performance callejera |

---

## Pódcasters y creadores de audio {#pódcasters-y-creadores-de-audio}

| Nombre | Discapacidad | Programa | Enfoque |
|------|------------|------|-------|
| Alice Wong | AME | Disability Visibility Podcast | Cultura de la discapacidad |
| Andrew Gurza | Parálisis cerebral | Disability After Dark | Discapacidad y sexualidad |
| Emily Ladau | Usuaria de silla de ruedas | The Accessible Stall | Noticias/cultura de la discapacidad |
| Kyle Khachadurian | Osteogénesis imperfecta | The Accessible Stall | Coconductor |
| Brianne Benness | Enfermedad crónica | No End in Sight | Enfermedad crónica |

---

## YouTubers y creadores de video {#youtubers-y-creadores-de-video}

Consulta [Canales de YouTube](/es/media/youtube-channels) para ver la lista detallada.

**Destacados:**

| Nombre | Discapacidad | Contenido |
|------|------------|---------|
| Squirmy and Grubs (Shane Burcaw) | AME | Relación entre una persona con discapacidad y una sin discapacidad |
| Jessica Kellgren-Fozard | Sorda, enfermedad crónica | Moda vintage, educación *(fuera de EE. UU.)* |
| Molly Burke | Ciega | Estilo de vida, maquillaje |
| Zach Anner | Parálisis cerebral | Comedia, viajes |
| Rikki Poynter | Sorda | Defensa de la accesibilidad |
| Annie Elainey | Enfermedad crónica | Justicia para las personas con discapacidad |

---

## Periodistas y ensayistas {#periodistas-y-ensayistas}

| Nombre | Discapacidad | Publicación/Enfoque |
|------|------------|-------------------|
| John Pring | Discapacidad física | Disability News Service *(fuera de EE. UU.)* |
| Imani Barbarin | Parálisis cerebral | Forbes, varios |
| s.e. smith | Enfermedad crónica, salud mental | Varias publicaciones |
| Vilissa Thompson | Usuaria de silla de ruedas | Ramp Your Voice |
| Keah Brown | Parálisis cerebral | Teen Vogue, varios |
| Lydia X. Z. Brown | Autista | Autistic Hoya, varios |

---

## Académicos y estudiosos {#académicos-y-estudiosos}

| Nombre | Discapacidad | Campo | Institución |
|------|------------|-------|-------------|
| Simi Linton | Usuaria de silla de ruedas | Estudios de la discapacidad | CUNY (emérita) |
| Rosemarie Garland-Thomson | Discapacidad física | Estudios de la discapacidad | Emory |
| Jay Dolmage | Discapacidad física | Retórica | University of Waterloo *(fuera de EE. UU.)* |
| Eli Clare | Discapacidad física | Estudios de la discapacidad/lo trans | Independiente |
| Lennard Davis | (Hijo de padres Sordos) | Estudios de la discapacidad | University of Illinois Chicago |
| Robert McRuer | Enfermedad crónica | Estudios queer/de la discapacidad | George Washington |
| Nirmala Erevelles | Discapacidad física | Estudios de la discapacidad | University of Alabama |
| Petra Kuppers | Enfermedad crónica | Performance/discapacidad | University of Michigan |

---

## Activistas que crean {#activistas-que-crean}

Muchas personas activistas de la discapacidad también son creadoras: escritoras, artistas, oradoras, hacedoras de medios.

| Nombre | Discapacidad | Obra creativa |
|------|------------|---------------|
| Alice Wong | AME | Escritura, pódcast, edición |
| Judy Heumann (1947-2023) | Polio | Memoria, conferencias |
| Haben Girma | Sordociega | Escritura, conferencias |
| Stella Young (1982-2014) | Osteogénesis imperfecta | Comedia, escritura *(fuera de EE. UU.)* |
| Imani Barbarin | Parálisis cerebral | Escritura, redes sociales |
| Vilissa Thompson | Usuaria de silla de ruedas | Escritura, consultoría |
| Leah Lakshmi Piepzna-Samarasinha | Enfermedad crónica | Escritura, performance |
| Colectivo Sins Invalid | Varias | Arte de performance |

---

## Cómo apoyar a las personas creadoras con discapacidad {#cómo-apoyar-a-las-personas-creadoras-con-discapacidad}

### Apoyo directo

- **Compra su obra** – Libros, arte, música, mercancía
- **Paga por el contenido** – Patreon, suscripciones, propinas
- **Contrátalas** – Conferencias, consultoría, encargos
- **Asiste a eventos** – Espectáculos, lecturas, performances

### Amplificación

- **Comparte su obra** – Redes sociales, recomendaciones
- **Da el crédito correcto** – Nómbralas, enlaza a sus plataformas
- **Reseña y recomienda** – Libros, espectáculos, álbumes
- **Pide accesibilidad** – Solicita a los espacios que contraten actos accesibles

### Cambio sistémico

- **Aboga por financiamiento** – Becas de arte para personas artistas con discapacidad
- **Impulsa la representación** – En los medios, la edición, las galerías
- **Cuestiona el control de acceso (gatekeeping)** – Espacios inaccesibles, instituciones capacitistas

---

## Contribuir

Para ser agregado a este directorio:

1. **Debes ser una persona con discapacidad** (se acepta la autoidentificación)
2. **Creas obra disponible públicamente**
3. **Incluye:** Nombre, discapacidad (si la compartes públicamente), medio, obra notable, sitio web/redes sociales

Para sugerir a otras personas creadoras:

1. **Deben ser abiertamente personas con discapacidad**
2. **Incluye la misma información anterior**
3. **Verifica que estén cómodas con figurar en la lista**

Consulta [Cómo Contribuir](/es/glossary/how-to-contribute)

---

## Páginas relacionadas

- [Libros](/es/media/books)
- [Canales de YouTube](/es/media/youtube-channels)
- [Personas creadoras en TikTok](/es/media/tiktok-creators)
- [Pódcasts](/es/media/podcasts)
- [Artes y performance de la discapacidad](/es/media/disability-arts-and-performance)

---

*Este directorio pone en el centro a personas creadoras con discapacidad en todos los medios.*

---

## Contribuye a esta página

¿Tienes experiencia vivida o conocimientos que podrían fortalecer esta página? Damos especial bienvenida a perspectivas sobre modelos poco representados aquí, incluidas las del Sur Global y de las comunidades indígenas.

[Sugiere una edición o un agregado →](/es/start/contribute)

---

*Esta página pone en el centro los conocimientos de las personas con discapacidad y se basa en la organización liderada por personas con discapacidad a nivel global. Para preguntas o para sugerir agregados, consulta [Cómo Contribuir](/es/start/contribute).*

*Última actualización: enero de 2026*
